                ...
                <ul className='w-full flex flex-wrap gap-4 justify-between mb-4 lg:w-[20%] lg:flex-col'>
                    {workExperiences.map((exp, index) => (
                        ...
                        <li key={exp.exp_id} className='flex justify-between pr-10'>    
                            <p className='uppercase text-[0.9rem] font-bold text-gray-400 cursor-pointer hover:text-gray-500 tansition transition-all duration-200 xs:text-[1rem]' onClick={() => {setActiveIndex(index)}}>{exp.organisation}</p>
                        </li> :
                        <li key={exp.exp_id} className='flex justify-between pr-10'>
                            <p className='uppercase text-[0.9rem] font-bold text-blue-900 border-b-[4px] border-[#0038BC] lg:border-none xs:text-[1rem]'>{exp.organisation}</p>
                            <div className='w-[4px] h-full bg-[#0038BC] hidden lg:block'></div>
                        </li>
                    ))}
                </ul>
                <div className='w-full h-[100%] lg:w-[80%]'>
                    <h1 className='font-bold text-[1rem] xs:text-[1.2rem]'>{workExperiences[activeIndex].role} @ <span className='text-blue-900'>{workExperiences[activeIndex].organisation}</span></h1>
                    <h2 className='uppercase mt-2 mb-8 text-[0.85rem] text-gray-500 xs:text-[0.9rem]'>{workExperiences[activeIndex].duration}</h2>
